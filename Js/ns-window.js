...
(function ($, window, undefined) {
    ...
    function _openAndCloseHandler(event) {
        ...
        if (event.type == $.nsWindowEvent.onOpen) {
            ...
            //$.nsWindow.bind($.nsWindowEvent.onClose, _openAndCloseHandler)
        }
    }

    function CloseDilog(sId) {
        try {
            var body = $(document.body);
            body.find('#' + sId).remove();
        } catch (ex) {
         
        }
        //SetBoxreturn();
        // body.find('.nswindowContainer')[iCount].removeNode();
    }

    function GetBoxreturn() {
        return _BoxReturn;
    }
    //-------------------------------------------------------------------------------------//
...
//            win.resizable();
        ...
        var sUrl = 'DilogBoxHolder.aspx?Pw=' + opt.width + '&Ph=' + opt.height + '&Pg=' + opt.dataUrl;
    ...
    function onSuccess(event, data) {
        //        var stringOfHtml = new Array(); // your string here
        //        stringOfHtml = jQuery171(data).filter("script");
        //        if (stringOfHtml.length > 0) {
        //            var fileref = document.createElement('script');
        //            fileref.setAttribute("type", "text/javascript");
        //            fileref.setAttribute("src", stringOfHtml[0].src);
        //            fileref.setAttribute("id", "JScript2");
        //            $("head")[0].appendChild(fileref);
        //        }
        setTimeout(function () { SetFormData(event, data); }, 1000);
        // event.dataContainer.innerHTML = data;
    }
    //-------------------------------------------------------------------------------------//
    function SetFormData(event, data) {
        event.dataContainer.innerHTML = data;
    }
